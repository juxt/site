...
(ns juxt.pass.openid-connect-test
  (:require
   ...
   [juxt.test.util :refer [*xt-node* *handler* with-xt with-handler]
    :as test-util])
  (:import
   (com.auth0.jwt JWT)
   (java.time Instant)
...
(deftest openid-auth0-login-redirect-test
  (with-redefs
   [slurp (tutils/mock-openid-configuration-req "auth0")]
    (site-init/install-openid-provider! *xt-node* "https://dev-14bkigf7.us.auth0.com/"))
...
(deftest openid-aws-cognito-login-redirect-test
  (with-redefs
   [slurp (tutils/mock-openid-configuration-req "aws-cognito")]
    (site-init/install-openid-provider! *xt-node* "https://cognito-idp.us-east-2.amazonaws.com/us-east-2_ccXNbbzY6"))
