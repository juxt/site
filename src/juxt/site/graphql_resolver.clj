...
(defn- form [args]
  (let [form (some-> args :juxt.site/request-context :ring.request/query (form-decode "US-ASCII"))]
    (if (string? form)
      {form "true"}
  ...
  (let [openapis
        (for [[uri api] (xt/q
                         db '{:find [openapi-uri openapi]
                              :where [[openapi-uri :juxt.apex/openapi openapi]]})]
          {:xt/id uri
           :type "OPENAPI"
           :contents api})
        graphql-apis
        (map first (xt/q db '{:find [uri]
                              :where [[uri :juxt.site/graphql-compiled-schema]]}))
        graphqls
    ...
    {"configuration"
     (fn [_]
       (let [config (repl/config)]
         {"baseUri" (:juxt.site/base-uri config)
          "unixPassPasswordPrefix"
          (:juxt.site.unix-pass/password-prefix config)
          "serverPortNumber"
          (get-in config [:ig/system
                          :juxt.site.server/server
                          :juxt.site/port])}))
     "database"
     (fn [_]
       (let [node (:juxt.site.db/xt-node system)
     ...
     "status"
     {"txLogAvail" (fn [_] (df (get-in (config) [:ig/system :juxt.site.db/xt-node :xtdb/tx-log :kv-store :db-dir])))
      "docStoreAvail" (fn [_] (df (get-in (config) [:ig/system :juxt.site.db/xt-node :xtdb/document-store :kv-store :db-dir])))
      "indexStoreAvail" (fn [_] (df (get-in (config) [:ig/system :juxt.site.db/xt-node :xtdb/index-store :kv-store :db-dir])))}
