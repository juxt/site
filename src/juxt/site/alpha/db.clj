...
(defn new-data-resource [uri resource-state]
  (let [last-modified (java.util.Date.)
    ...
    (concat
     ...
     (let [rep-id (java.util.UUID/randomUUID)]
       [
...
(defn user-entity [username password]
  (new-data-resource
   (java.net.URI. (format "/_crux/users/%s" username))
    ...
    (mapcat
     ...
     (for [fl (.listFiles dir)
           :let [[_ suffix] (re-matches #".*\.(.*)" (.getName fl))
                 uri (java.net.URI. (format "/_crux/swagger-ui/%s" (.getName fl)))
                 rep-id (java.util.UUID/randomUUID)]]
       [;; Resource
        [:crux.tx/put
         {:crux.db/id uri
          ::spin/methods #{:get :head :options}}]
...
(defn seed-database! [crux-node]
  (crux/submit-tx
   ...
   (concat
    ...
    ;; The crux/admin user
    (user-entity "crux/admin" "FunkyForest")))
