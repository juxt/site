...
Site supports two broad categories of user authentication:
...
. (((authentication,cookie-based))) Cookie-based authentication, which we
  describe in <<ch-cookie-authentication>>, opens up numerous additional options
  for authentication. One such option is OpenID Connect, which is covered in
  <<ch-openid>> with a working example.
