...
(defn perform-mutation!
  ([opts]
   (perform-mutation! opts false))
  ([{:keys [argument-values site-args xt-node db base-uri fields
            lookup-entity field-kind pass/subject user-id type-k]
     :as opts} skip-synergy-event?]
            ...
            (let [result (get object-value (keyword field-name))]
              (cond
                ...
                (limit-results argument-values result)
                 ;; TODO validate enum
                (enum? (field->type field) types-by-name)
                (str result)
                :else
                result))
...
(defn post-handler [{::site/keys [uri db]
  ...
  (let [schema (some-> (xt/entity db uri) ::site/graphql-compiled-schema)
        ...
        {query "query"
         ...
         variables "variables"}
        (case (some-> req ::site/received-representation ::rfc7231/content-type ::rfc7231/subtype)
          "json" (try (some-> body json/read-value)
                      (catch Exception e
                        (throw
        ...
        variables (if (map? variables) variables (json/read-value variables))
        variables (into
