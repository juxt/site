...
Site maps the concept of an XTDB entity onto the concept of a REST resource.

[quote,Roy Fielding,https://www.ics.uci.edu/~fielding/pubs/dissertation/rest_arch_style.htm]
--
The key abstraction of information in REST is a resource. Any information that can be named can be a resource: a document or image, a temporal service (e.g. "today's weather in Los Angeles"), a collection of other resources, a non-virtual object (e.g. a person), and so on. In other words, any concept that might be the target of an author's hypertext reference must fit within the definition of a resource. A resource is a conceptual mapping to a set of [.line-through]#entities#(XTDB documents), not the [.line-through]#entitys#(XTDB document) that corresponds to the mapping at any particular point in time.

More precisely, a resource latexmath:[R] is a temporally varying membership function latexmath:[M{\text{R}}(t)], which for time t maps to a set of [.line-through]#entities#(XTDB documents), or values, which are equivalent.
--

An entity shares a number of and a web resource have a number of features in common.
...
So, it's _that_ capability that Site adds to an entity.
