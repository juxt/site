...
(ns juxt.site.graphql
  (:require
   [ring.util.codec :refer [form-decode]]
   [selmer.parser :as selmer]
   [juxt.grab.schema :as schema]
   [juxt.grab.document :as document]
   [jsonista.core :as json]
   [juxt.grab.execution :refer [execute-request]]
   [juxt.grab.parser :as parser]
   [juxt.site.util :refer [assoc-some]]
...
(defn- args-to-entity
  ...
  (let [args argument-values
        types-by-name (:juxt.grab.schema/types-by-name schema)
...
(defn query [schema document operation-name variable-values
  ...
  (let [type-k
        (or
         (some-> schema :juxt.grab.schema/directives (get "site") ::g/arguments (get "type") keyword)
          ...
          (cond
            ...
            (let [resolver
                  (case (get site-args "siteResolver")
                    "allQueryParams"
                    (requiring-resolve 'juxt.site.graphql-resolver/query-parameters)
                    "queryParam"
                    (requiring-resolve 'juxt.site.graphql-resolver/query-parameter)
                    "queryString"
                    (requiring-resolve 'juxt.site.graphql-resolver/query-string)
                    "constant"
                    (requiring-resolve 'juxt.site.graphql-resolver/constant)
...
(defn put-handler [{::site/keys [uri db xt-node] :as req}]
  (let [schema-str (some-> req :juxt.site/received-representation :juxt.http/body (String.))
...
(defn stored-document-put-handler [{::site/keys [uri db xt-node] :as req}]
  (let [document-str (some-> req :juxt.site/received-representation :juxt.http/body (String.))
    ...
    (when-not (::site/graphql-schema resource)
      (throw
       (ex-info
        "Resource should have a :juxt.site/graphql-schema key"
...
(defn text-html-template-model [{::site/keys [resource db]}]
  (let [original-resource (if-let [variant-of (::site/variant-of resource)] (xt/entity db variant-of) resource)
        endpoint (:juxt.site/graphql-schema original-resource)
        ...
        operation-names (->> (:juxt.grab.document/operations document)
