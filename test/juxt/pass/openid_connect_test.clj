...
; login tokens
  (testing
   (let [resp (with-redefs
     ...
     (is (= (xtdb/entity db "https://example.org/_site/users/exceladmin")
            {:xt/id "https://example.org/_site/users/exceladmin"
             ::site/type "User"
             ::pass/username "exceladmin"
             :name "Excel Admin"
             :email "fwc+exceladmin@juxt.pro"}))
     (is (= (xtdb/entity db "https://example.org/_site/users/exceladmin/oauth-credentials")
     ...
     (is (some? (xtdb/entity db "https://example.org/_site/roles/superuser")))
     (is (= (xtdb/entity db "https://example.org/_site/roles/superuser/users/exceladmin")
   ...
   (let [resp (with-redefs
     ...
     (is (= (xtdb/entity db "https://example.org/_site/users/exceladmin/oauth-credentials")
            {:xt/id "https://example.org/_site/users/exceladmin/oauth-credentials"
             ...
             :juxt.pass.jwt/iss "https://cognito-idp.us-east-2.amazonaws.com/us-east-2_ccXNbbzY6"
             :juxt.pass.jwt/sub "2353661c-e432-464f-9b22-e908ddd920e5"}))
     (is (nil? (xtdb/entity db "https://example.org/_site/roles/superuser")))
     (is (nil? (xtdb/entity db "https://example.org/_site/roles/superuser/user/exceladmin"))))))
